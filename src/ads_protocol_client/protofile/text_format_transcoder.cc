#include "text_format_transcoder.h"

#include <memory>
#include <string>


#include "file_descriptor_info.h"
...
bool TextFormatTranscoder::parseInto(std::string text_format,
...
bool TextFormatTranscoder::toBinarySerializationInternal(
    std::string type_name, google::protobuf::io::ZeroCopyInputStream* input_stream,
...
std::unique_ptr<google::protobuf::Message> TextFormatTranscoder::createEmptyDynamicMessage(
    std::string type_name, google::protobuf::io::ErrorCollector* error_collector) const {
  ...
  if (descriptor == nullptr) {
    if (error_collector) {
      //error_collector->RecordError(0, 0, absl::StrFormat("Could not find descriptor for: %s", type_name));
    }
    return nullptr;
  }

/*  auto dynamic_message = absl::WrapUnique(internals_->message_factory.GetPrototype(descriptor)->New());
  if (!dynamic_message) {
    if (error_collector) {
      error_collector->RecordError(0, 0, absl::StrFormat("Could not create dynamic message for: %s", type_name));
    }
    return nullptr;
  }
